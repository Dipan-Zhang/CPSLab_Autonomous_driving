...
#include "pid_simulator/pose_2d.hpp"
#include "pid_simulator/ackermann_kinematics.hpp"
#include "pid_simulator/scan_simulator_2d.hpp"

#include "pid_simulator/car_state.hpp"
#include "pid_simulator/car_params.hpp"
#include "pid_simulator/ks_kinematics.hpp"
#include "pid_simulator/st_kinematics.hpp"
#include "pid_simulator/precompute.hpp"
...
private:
    ...
    // Lap completed
    float previous_state_x = 0.0;
    ros::Time measure_last;
    bool is_first_drive_command = true;
...
public:
    ...
    RacecarSimulator(): im_server("racecar_sim") {
        ...
        // Make a publisher for lap time broadcasting
        lap_pub = n.advertise<std_msgs::Duration>("/lap", 1);
            ...
            if (state.velocity != 0) {
                for (size_t i = 0; i < scan_.size(); i++) {
                    ...
                    if ((ttc < ttc_threshold) && (ttc >= 0.0)) {
                        ...
                        // Reset race system (i.e., teleport car, reset time and first command boolean)
                        is_first_drive_command = true;
                        state = {.x=0, .y=0, .theta=0, .velocity=0, .steer_angle=0.0, .angular_velocity=0.0, .slip_angle=0.0, .st_dyn=false};
                    }
                }
            }
    ...
    void drive_callback(const ackermann_msgs::AckermannDriveStamped & msg) {
        ...
        if (is_first_drive_command) {
            measure_last = ros::Time::now();
            is_first_drive_command = false;
        }
    }
    ...
    void clear_obstacles(const visualization_msgs::InteractiveMarkerFeedbackConstPtr &feedback) {
        ...
        void pub_pose_transform(ros::Time timestamp) {
            ...
            if ((-1.5 < state.y) & (state.y < 1.5) & (previous_state_x < 0) & (0 < state.x)) {
                ros::Time measure_now = ros::Time::now();
                std_msgs::Duration time_measure_lap;
                time_measure_lap.data = measure_now-measure_last;
                ROS_INFO("Lap completed in %is and %ins!", time_measure_lap.data.sec, time_measure_lap.data.nsec);
                lap_pub.publish(time_measure_lap);
                measure_last = measure_now;
            }
            previous_state_x = state.x;
